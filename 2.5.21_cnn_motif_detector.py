...
"""
r2 = pd.read_csv("C:\\Users\\makow\\Documents\\Research\\Data Analysis\\2.6.21_97_mat_NGS\\R2.csv", header = None, index_col = None)
indices = []
for index, row in r2.iterrows():
    ...
    if 'X' in list(row[0]):
        indices.append(index)
r2.drop(r2.index[indices], inplace = True)
r2.set_index(0, inplace = True)
r3 = pd.read_csv("C:\\Users\\makow\\Documents\\Research\\Data Analysis\\2.6.21_97_mat_NGS\\R3.csv", header = None, index_col = None)
indices = []
for index, row in r3.iterrows():
    ...
    if 'X' in list(row[0]):
        indices.append(index)
r3.drop(r3.index[indices], inplace = True)
r3.set_index(0, inplace = True)
r5 = pd.read_csv("C:\\Users\\makow\\Documents\\Research\\Data Analysis\\2.6.21_97_mat_NGS\\R5.csv", header = None, index_col = None)
indices = []
for index, row in r5.iterrows():
    ...
    if 'X' in list(row[0]):
        indices.append(index)
r5.drop(r5.index[indices], inplace = True)
r5.set_index(0, inplace = True)
r6 = pd.read_csv("C:\\Users\\makow\\Documents\\Research\\Data Analysis\\2.6.21_97_mat_NGS\\R6.csv", header = None, index_col = None)
indices = []
for index, row in r6.iterrows():
    ...
    if 'X' in list(row[0]):
        indices.append(index)
r6.drop(r6.index[indices], inplace = True)
r6.set_index(0, inplace = True)
r7 = pd.read_csv("C:\\Users\\makow\\Documents\\Research\\Data Analysis\\2.6.21_97_mat_NGS\\R7.csv", header = None, index_col = None)
indices = []
for index, row in r7.iterrows():
    if len(list(row[0])) != 141:
        indices.append(index)
    if '*' in list(row[0]):
        indices.append(index)
    if 'X' in list(row[0]):
        indices.append(index)
r7.drop(r7.index[indices], inplace = True)
r7.set_index(0, inplace = True)

neg = r2[~r2.index.isin(r3.index)]
neg = neg[~neg.index.isin(r5.index)]
neg = neg[~neg.index.isin(r6.index)]
neg = neg[~neg.index.isin(r7.index)]
neg.reset_index(drop = False, inplace = True)

pos = pd.concat([r2, r3, r5, r6, r7], axis = 1, ignore_index = False)
pos.dropna(inplace = True)
pos.reset_index(drop = False, inplace = True)

neg = neg.iloc[0:1134,:]
neg['label'] = [0]*1134
neg.columns = ['sequence', 'frequency', 'label']
pos = pos.iloc[:1134,:]
pos['label'] = [1]*1134
pos.columns = ['sequence', 'frequency2', 'frequency3', 'frequency5', 'frequency6', 'frequency7','label']
